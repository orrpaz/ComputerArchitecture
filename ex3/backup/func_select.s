    ...
    xor          %rax, %rax
    call         pstrlen
    movq         %rax, %r15                   #get the return value into temporary 

    movq        %rdx, %rdi                    #get the second pstring to rdi - first argument
    xor         %rax, %rax
    call        pstrlen
    movq        %rax, %r14                    #get the return value into temporary
    
    movq        $stringCase50, %rdi           #format into rdi
    movq        %r15, %rsi                    #first length into rsi
    movq        %r14, %rdx                    #second length into rdx
    xor         %rax, %rax
    ...
    movq        %r14,%rdi 		  # rdi has pointer to first string
    ...
    # print
    movq        $stringCase51,%rdi 	 # rdi hold the format string - first arg
    movzbq      (%rsp),%rdx		 # new char in rdx - third arg    
    incq        %rsp 		         # old char in rsi -second arg
    movzbq      (%rsp),%rsi    
    ...
    movzbq      (%rbx), %rsi            #the length into rsi
    addq        $1, %rbx                #point onlt to the string
    movq        %rbx, %rdx              #get the string into parameter for printf
    xor         %rax, %rax
    call        printf
