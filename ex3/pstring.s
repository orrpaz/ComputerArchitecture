...
pstrijcpy:
    ...
    cmpb 	(%r12),%cl 		# check j is valid in dst (part of rcx ,cl, contain j) - I dont know if we want to consider '\0' as a letter.
                                          # if yes, we use in 'ja' ,otherwise we use in 'jae' 
                                          # for example: we insert 7 , $%#rkfe , 4 s#$R , 52 with index [i=1,j=4]
                                          # the question is if '\0' is part of string or not. 
...
swapCase:
    ...
    movq         %rdi,%rax              # move the pstring on rax
    ...
    cmpb        $65, (%rdi)            
    jl          .LoopSwapCase            # it is not letter
    cmpb        $122, (%rdi)
    jg          .LoopSwapCase
