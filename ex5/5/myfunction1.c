...
void smoothBlur(pixel *src) {
    ...
    register int iiStart;
    register int calcIndexRet = 0;
    register int red;
    register int green;
    register int blue;
    ...
    for (i = 1; i < size; i++) {
        ...
        for (j = 1; j < size; j++) {
            ...
            calcIndexRet = (iiStart) * (dim) + (j-1);
            p1 = src[calcIndexRet];
            p2 = src[calcIndexRet + 1];
            p3 = src[calcIndexRet + 2];
            red += ((int) p1.red);
            green += ((int) p1.green);
            blue += ((int) p1.blue);
            red += ((int) p2.red);
