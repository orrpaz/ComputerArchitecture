...
//register it's a hint to the compiler that the variable will be heavily used and recommend it be kept in a processor register if possible.
    ...
    for (i = 1; i < size; ++i) {
        ...
        for (j = 1; j < size; ++j) {
            ...
            red += ((int) p1.red) + ((int) p2.red) + ((int) p3.red);
            green += ((int) p1.green) + ((int) p2.green) + ((int) p3.green);
            blue += ((int) p1.blue) + ((int) p2.blue) + ((int) p3.blue);
...
void copyPixels(pixel* dst) {
    ...
    register int row;
    register int col;
    register int nm = nLocal*mLocal;
    register int decreseNmFromCol =0;

    for (row = mLocal ; row > 0 ; --row) {
        for (col = nLocal ; col > 0 ; --col) {
            decreseNmFromCol = nm-col;
            dst[decreseNmFromCol]= src[decreseNmFromCol];
        }
        // decresae n from n*m
        nm-=nLocal;
    }
